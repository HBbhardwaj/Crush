...
import 'package:crush/modal/sent_model.dart';
...
class Sent extends StatefulWidget {
  var userId;
   Sent({ Key? key,required this.userId}) : super(key: key);

  @override
  State<Sent> createState() => _SentState(this.userId);
}

class _SentState extends State<Sent> {
  var userId;
  _SentState(this.userId);

late Future<SentModal> msghData;
  int _Statuscode = 0;
void initState() {
    super.initState();
      msghData = Allmsg();
 ...
 Future<SentModal> Allmsg() async {
    var request = http.MultipartRequest('POST', Uri.parse(Apis.sent_message));
  ...
  'sender_id': userId
    ...
    if (response.statusCode == 200) {
      ...
      return SentModal.fromJson(json.decode(response.body));
    } else {
      print(response.body);
    }
    return SentModal.fromJson(jsonDecode(response.body));
  }
